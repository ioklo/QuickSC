...
namespace QuickSC.StaticAnalyzer
{
    ...
    partial class QsExpAnalyzer
    {
        class MemberCallExpAnalyzer
        {
            ...
            {
                public QsTypeValue_Func TypeValue { get; }
                public QsMemberCallExpInfo NodeInfo { get; }
                public ImmutableArray<QsTypeValue> ArgTypeValues { get; }

                public Result(QsTypeValue_Func typeValue, QsMemberCallExpInfo nodeInfo, ImmutableArray<QsTypeValue> argTypeValues)
                {
                    TypeValue = typeValue;
                    NodeInfo = nodeInfo;
            ...
            public MemberCallExpAnalyzer(QsExpAnalyzer expAnalyzer, QsMemberCallExp exp, QsAnalyzerContext context)
            {
            ...
            {
                var memberTypeArgs = QsAnalyzerMisc.GetTypeValues(exp.MemberTypeArgs, context);

                // 1. 함수에서 찾기.. FuncValue도 같이 주는것이 좋을 듯 하다
                if (context.TypeValueService.GetMemberFuncValue(objTypeValue, QsName.Text(exp.MemberName), memberTypeArgs, out var funcValue))
                {
                    // staticObject인 경우는 StaticFunc만, 아니라면 모든 함수가 가능 
                    var funcTypeValue = context.TypeValueService.GetTypeValue(funcValue);

                    var nodeInfo = QsAnalyzerMisc.IsFuncStatic(funcValue.FuncId, context)
                ...
                {
                    if (context.TypeValueService.GetMemberVarValue(objTypeValue, QsName.Text(exp.MemberName), out var varValue))
                    {
                        // TODO: as 대신 함수로 의미 부여하기, 호출 가능하면? 쿼리하는 함수로 변경
                        var varFuncTypeValue = context.TypeValueService.GetTypeValue(varValue) as QsTypeValue_Func;
            ...
            private Result? Analyze_Type(QsAnalyzerIdentifierInfo.Type typeIdInfo)
            {
                ...
                // 1. 함수에서 찾기
                if (context.TypeValueService.GetMemberFuncValue(objTypeValue, QsName.Text(exp.MemberName), memberTypeArgs, out var memberFuncValue))
                {
                    if (!QsAnalyzerMisc.IsFuncStatic(memberFuncValue.FuncId, context))
                    {
